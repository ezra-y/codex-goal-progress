...
- 🪦 移除了 Codex 写的那个两秒循环屎山；Goal Progress 现在响应真实的任务和连接事件。
- 🔄 页面 Reload 后回到任务时，进度会自动恢复。
- 🧭 即使 Codex 侧栏暂时使用临时 ID，新任务也能正常工作。
- 🔌 当前 Codex 页面 Reload 或短暂断开时，Goal Progress 会立即开始恢复。
- ✨ “已暂停”和“总进度”的字重现在会更自然地贴合 Codex。
