...
- 🧩 已适配最新版 Codex 的 Goal 行、侧栏任务与输入区结构。
- 🫥 Goal 完成后立即清除进度界面，不再残留 100% 卡片。
- 📌 输入框内容再长，沉底进度仍留在已经验证的原生位置。
- 👀 可见性诊断现在与屏幕上真实可见的结果一致。
- 🧬 新 Goal 不会再错误继承上一个 Goal 的进度。
- ⏱️ 状态文字与计时变化不再让当前 Goal 被误判成新 Goal。
- 📐 沉底进度会实时跟随原生 Goal 的左右边界。
- 🧹 更新后会清理旧 Plugin 缓存，不再残留旧 Hook 与 Skill 文件。
- 🔌 兼容更新时，正在运行的 Goal Progress 无需重启 Codex 也能继续工作。
