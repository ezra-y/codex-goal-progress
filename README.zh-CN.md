...
- 安装源码插件后，在本机构建 Helper；预构建 macOS 安装包继续提供。
- Contract ID 由插件生成，并在正常重启 Codex 后恢复进度。
- 大尺寸错误卡片改为一行提示，保留上次确认的进度、重试和关闭。
- 区分任务受阻与插件故障，并更新到 `fast-uri` 3.1.7 和 `qs` 6.16.0。

源码插件和预构建安装包现在使用同一套 v0.3.5 Renderer 与修复。
