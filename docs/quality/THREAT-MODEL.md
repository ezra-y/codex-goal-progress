...
| release 和 Plugin | 安装前验证版本、SHA-256、Plugin 全树清单和 Hook 内容；ad-hoc codesign 只检查构建完整性 |
...
| release 或 Plugin 被改 | SHA256SUMS、Release manifest、Plugin 全树路径/SHA/mode 和 Hook hash；ad-hoc codesign 不证明发布者身份 | `tests/integration/release-plugin-install.test.ts` |
